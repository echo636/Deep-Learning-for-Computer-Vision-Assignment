...
def nn_forward_backward(
    ...
    loss = -torch.sum(torch.log(correct_probs))
    loss /= N
    ...
    dscore /= N
    grads['W2'] = h1.t() @ dscore + 2 * reg * W2
    grads['b2'] = torch.sum(dscore, dim=0)
    dh1 = dscore @ W2.t()
    dz1 = dh1 * (h1 > 0)
...
def nn_predict(
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    scores = (X @ params['W1'] + params['b1']) @ params['W2'] + params['b2']
      ...
      for h in hidden_sizes:
        for reg in reg_strengths:
          for decay in lr_decays:
            ...
            C = data_dict['y_train'].shape[0]
            params = {
              'W1': torch.randn(D, h) * (2/D)**0.5,
              'b1': torch.zeros(h),
              'W2': torch.randn(h, C) * (2/h)**0.5,
              'b2': torch.zeros(C)
            }
            ...
            current_val_acc = stats['val_acc_history'][-1]
            if current_val_acc > best_val_acc:
              best_val_acc = current_val_acc
              best_stat = stats
              best_net = {k: v.clone() for k,v in params.items()}
    #############################################################################
    #                               END OF YOUR CODE                            #
    #############################################################################
