...
def svm_loss_vectorized(
    ...
    margin[margin[:, :] < 0] = 0
    loss += torch.sum(margin.view(-1)) / num_train
    ...
    #############################################################################
    # Replace "pass" statement with your code
    margin_mask = (margin[:, :] > 0).double()
    margin_mask[torch.arange(num_train), y] = -torch.sum(margin_mask, dim = 1)
    dW = X.t() @ margin_mask / num_train
    dW += 2 * reg * W
    #############################################################################
    #                             END OF YOUR CODE                              #
    #############################################################################
...
def test_one_param_set(
    ...
    # Replace "pass" statement with your code
    cls.train(data_dict['X_train'], data_dict['y_train'], learning_rate=lr, reg=reg)
    y_pred_val = cls.predict(data_dict['X_val'])
    y_pred_train = cls.predict(data_dict['X_train'])
    train_acc = (y_pred_train == data_dict['y_train']).float().mean().item()
    val_acc = (y_pred_val == data_dict['y_val']).float().mean().item()
    ############################################################################
    #                            END OF YOUR CODE                              #
    ############################################################################
...
def softmax_loss_naive(
    ...
    C = W.shape[1]
    for i in range(N):
      loss += -torch.log(exp_score[i][y[i]] / torch.sum(exp_score[i]))
      for j in range(C):
        prob = exp_score[i][j] / torch.sum(exp_score[i])
        if j == y[i]:
          dW[torch.arange(N), j] += (prob - 1) * X[i] / N
        else:
          dW[torch.arange(N), j] += prob * X[i] / N
...
def softmax_loss_vectorized(
    ...
    exp_score = torch.exp(score)
    N = X.shape[0]
    probs = exp_score / torch.sum(exp_score, dim=1)
    log_score = -torch.log(exp_score[torch.arange(N), y] / torch.sum(exp_score, dim=1)) 
    loss = torch.sum(log_score) / N
    exp_score[torch.arange(N), y] -= 1
    dW = X.t() @ exp_score / N
    loss += reg * torch.sum(W * W)
    dW += 2 * reg * W
    #############################################################################
