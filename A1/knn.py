...
def compute_distances_two_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    x1 = 1
    x2 = 1
    for i in x_train.shape:
      x1 *= i
    for i in x_test.shape:
      x2 *= i
    x1 //= num_train
    x2 //= num_test
    x_train = x_train.view(num_train, x1)
    x_test = x_test.view(num_test, x2)
...
def compute_distances_one_loop(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    num1 = x_train.numel()
    num2 = x_test.numel()
    t1 = x_train.view(num_train, num1 // num_train).clone()
    t2 = x_test.view(num_test, num2 // num_test).clone()
    for i in range(num_train):
      dists[i] = ((((t1[i] - t2) ** 2).sum(dim = 1)) ** 0.5)
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    num1 = x_train.numel()
    num2 = x_test.numel()
    t1 = x_train.view(num_train, num1 // num_train)
    t2 = x_test.view(num_test, num2 // num_test)
    p2 = torch.mm(t1, t2.t())
    t1 = (t1 ** 2).sum(dim=1)
    t2 = (t2 ** 2).sum(dim=1)
...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    for i in range(num_test):
      v, idx = torch.topk(dists[::, i], k, largest=False)
      a = y_train[idx]
      label, count = torch.unique(a, return_counts = True)
      max_count = torch.max(count)
      x = label[count == max_count]
      y_pred[i] = torch.min(x).item()
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return y_pred
...
def knn_cross_validate(
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    x1 = torch.chunk(x_train, num_folds)
    y1 = torch.chunk(y_train, num_folds)
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    ...
    KnnClassifier.__init__(x_train[0: p1 - 1], y_train[0: p1 - 1])
