...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        out1, cache1 = L.forward(X, self.params['W1'], self.params['b1'])
        out2, cache2 = R.forward(out1)
        out3, cache3 = L.forward(out2, self.params['W2'], self.params['b2'])
        scores = out3
        ##############################################################
        #                     END OF YOUR CODE                       #
        ##############################################################
        ...
        dout3, grads['W2'], grads['b2'] = L.backward(dscore, cache3)
        grads['W2'] += 2 * self.reg * self.params['W2']

        dout2 = R.backward(dout3, cache2)

        dout1, grads['W1'], grads['b1'] = L.backward(dout2, cache1)
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for i in range(self.num_layers):
          ...
          self.params[f'b{i+1}'] = torch.randn(dims[i+1], dtype=dtype, device=device)
        #######################################################################
        #                         END OF YOUR CODE                            #
        #######################################################################
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers - 1):
          out, cache = L.forward(out, self.params[f'W{i+1}'], self.params[f'b{i+1}'])
          caches.append(cache)
          
          out, cache = R.forward(out)
          caches.append(cache)
        ...
        for i in range(self.num_layers, 1, -1):
          dout, grads[f'W{i}'], grads[f'b{i}'] = L.backward(dout, caches.pop()) #???
          dout = R.backward(dout, caches.pop())
